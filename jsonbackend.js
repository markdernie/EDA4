...
http.createServer((req, res) => {
    ...
    if (req.method == "PUT") {
        ...
        req.on('end', () => {
            ...
            //let index = '{"id":"' + bodyparsed.id  +",lastupdate"+ '"}'
        ...
        fs.readFile('./jsonfiles/file1.dat', "utf8", function (err, data) {
           // printTime('fs.readfile outer', indexfile,data,'readfile:','data:',verbose)
            if (!data) {
                ...
                uniquelines.map((val)=>{let pars=JSON.parse(val);console.log('id:',pars.id)})
                let ids=uniquelines.map((val)=>{let pars=JSON.parse(val);console.log(pars.id);return pars.id})
                console.log('ids:',ids)
                let returnval=[]
